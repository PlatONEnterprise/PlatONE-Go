...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "bcos-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/BCOSnetwork/BCOS-Go/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "bcos.aar"), "bcos.aar", os.ModePerm)
