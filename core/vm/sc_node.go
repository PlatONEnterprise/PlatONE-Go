...
func (n *SCNode) VrfElection(nonce []byte) (int32, error) {
	...
	if vrf.ElectionEpoch == 0 ||
		...
		n.emitEvent("no need for election", operateSuccess, fmt.Sprint(vrf.ElectionEpoch))
		return 0, nil
	}
	...
	n.emitEvent("consensusNodeInfo", operateSuccess, fmt.Sprint(len(consensusNodes)))
