...
* INPUT：
  ...
  * page_id（uint64）[必须]：页号。从1开始。 //page_index?
  * page_num（uint64）[必须]：每页多少合约数量。不限制最大最小值。 //page_size?
...
* OUTPUT：
  ...
  ```json
  {
    ...
    "items": [
    	{
        "contract_address":"0xab123", //合约地址

        //缺少contract_name、creator、create_tx、timestamp
      }
    ],
  }
  ```
...
* EXAMPLE：
  ...
  ```shell
  curl http://localhost:7000/contracts?page_id=1&page_num=50
  ```
