...
The * is stand for all account addresses or APIs`,
	...
	FwDeleteCmd = cli.Command{
		...
		Flags:     globalCmdFlags,
		Description: `
		ctool fw delete <address> <action> <account> <api>`,
	}
	...
	FwResetCmd = cli.Command{
		...
		Flags:     globalCmdFlags,
		Description: `
		ctool fw reset <address> <action> <account> <api>`,
	}
...
func fwImport(c *cli.Context) {
	...
	result := innerCall(c, funcName, funcParams, packet.FW_TX_TYPE)
	fmt.Printf("result: %s\n", result)
}
