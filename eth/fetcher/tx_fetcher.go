...
func (f *TxFetcher) Enqueue(peer string, txs []*types.Transaction, direct bool) error {
	...
	// Push all the transactions into the pool
	var (
	...
	for i, err := range errs {
		if err != nil {
			...
			case core.ErrTransactionRepeat:
				duplicate++
...
func (f *TxFetcher) loop() {
	var (
		...
		fetchTimeoutTimer = new(mclock.Timer)

		waitTrigger         = make(chan struct{}, 1)
		fetchTimeoutTrigger = make(chan struct{}, 1)
	)
	for {
		select {
		case ann := <-f.notify:
			...
			if !oldPeer && len(f.announces[ann.origin]) > 0 {
				f.scheduleFetches(fetchTimeoutTimer, fetchTimeoutTrigger, map[string]struct{}{ann.origin: {}})
			}
		...
		case <-waitTrigger:
			...
			if len(actives) > 0 {
				f.scheduleFetches(fetchTimeoutTimer, fetchTimeoutTrigger, actives)
			}

		case <-fetchTimeoutTrigger:
			...
			f.scheduleFetches(fetchTimeoutTimer, fetchTimeoutTrigger, nil)

			// No idea if we scheduled something or not, trigger the timer if needed
			f.rescheduleTimeout(fetchTimeoutTimer, fetchTimeoutTrigger)
		...
		case delivery := <-f.cleanup:
			...
			if delivery.direct {
				...
				f.scheduleFetches(fetchTimeoutTimer, fetchTimeoutTrigger, nil) // Partial delivery may enable others to deliver too
			}
