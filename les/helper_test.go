...
func testChainGen(i int, block *core.BlockGen) {
	...
	case 0:
		...
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), acc1Addr, big.NewInt(10000), params.TxGas, nil, nil, common.DefaultTxType), signer, testBankKey)
		block.AddTx(tx)
	case 1:
		...
		tx1, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), acc1Addr, big.NewInt(1000), params.TxGas, nil, nil, common.DefaultTxType), signer, testBankKey)
		tx2, _ := types.SignTx(types.NewTransaction(nonce, acc2Addr, big.NewInt(1000), params.TxGas, nil, nil, common.DefaultTxType), signer, acc1Key)
	...
	case 2:
		...
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), testContractAddr, big.NewInt(0), 100000, nil, data, common.DefaultTxType), signer, testBankKey)
		block.AddTx(tx)
	case 3:
