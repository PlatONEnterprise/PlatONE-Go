...
func main() {
	...
	for {
		...
		tx, err := types.SignTx(types.NewTransaction(nonces[index], crypto.PubkeyToAddress(faucets[index].PublicKey), new(big.Int), 21000, big.NewInt(100000000000), nil, common.DefaultTxType), types.HomesteadSigner{}, faucets[index])
		if err != nil {
			panic(err)
		}
